...
class Trainer(basic.Trainer):
    ...
    def set_momentum_params(self, average_momentum):
        if not average_momentum is None:
            for group, mo_group in zip(self.optimizer.param_groups, average_momentum):
                # through the models trained by the opt
                for p, m in zip(group['params'], mo_group):
                    param_state = self.optimizer.state[p]
                    param_state['momentum_buffer'] = torch.clone(m).detach()
    ...
    def get_momentum_params(self):
        ...
        for group in self.optimizer.param_groups:
            ...
            for p in group['params']:
                momentum.append(self.optimizer.state[p]['momentum_buffer'])
