...
class ScaffoldOptimizer(optim.SGD):
    ...
    def step(self, init_grad, closure=None):
        ...
        for group in self.param_groups:
            ...
            idx = 0
            for p, server_update_direction, grad in zip(
                    group['params'], self.server_update_direction, init_grad):
                if p.grad is None:
                    continue
                d_p = p.grad.data
                ...
                d_p.add_(grad, alpha=-1)
                d_p.add_(server_update_direction, alpha=1 / group['lr'] / self.batches_num)
                # self.server_update_direction[idx] = server_update_direction * (1 - (1 / self.batches_num)) +\
                #                                     d_p_bak * (1 / self.batches_num) * group['lr']
                idx += 1
