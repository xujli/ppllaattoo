...
class Trainer(basic.Trainer):
    ...
    def topk(self):
        for name, params in self.model.parameters_and_names():
            params = self.sparsify_topk(params, k=max(int(params.numel() * self.topk_ratio), 1))

    def sparsify_topk(self, input_tensor):
        topk_sparse_tensor = torch.topk(torch.abs(input_tensor.flatten()), k)
        sparse_tensor = torch.sparse_coo_tensor(topk_sparse_tensor.indices.view(1, -1),
                                                input_tensor.flatten()[topk_sparse_tensor.indices],
                                                input_tensor.flatten().shape)
        dense_tensor = sparse_tensor.to_dense().reshape(input_tensor.shape)
        return dense_tensor
