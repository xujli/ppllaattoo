    ...
    def step(self, closure=None):
        ...
        for group in self.param_groups:
            ...
            for p, server_update_direction in zip(
                ...
                # d_p.mul_(self.alpha).add_(server_update_direction, alpha=1 / group['lr'] / self.batches_num)
                # coef = (1 - (1 / self.batches_num)) * (1 / self.interval)
                # self.server_update_direction[idx] = server_update_direction * coef + d_p_bak * group['lr']
                # print(len(self.server_update_direction), idx, self.interval, momentum)
                idx += 1
                # Update weight
