...
mean_acc, mean_std = [], []
alphas = []
alphasss = np.arange(0, 10).astype(np.float)
alphasss[0] = 0.1
...
scatter = plt.scatter(alphasss, max_acc, s=max_std * np.pi * 100,
            alpha=0.8, color=cmap_list3[0], label='Test Accuracy')
...
plt.twinx()
plt.scatter(alphasss, mean_acc, s=mean_std * 40,
         alpha=0.8, color=cmap_list3[2], label='# of Rounds')
...
plt.savefig('./Test Accuracy via different alpha {}.pdf'.format(alpha))
plt.show()