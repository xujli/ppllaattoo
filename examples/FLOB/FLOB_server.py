...
class Server(fedavg.Server):
    ...
    def extract_client_updates(self, updates):
        ...
        updates = self.algorithm.compute_weight_updates(weights_received)
        ...
        self.adaptive_weighting = self.calc_adaptive_weighting(updates, num_samples)
    ...
    def calc_contribution(self, updates, num_samples):
        ...
        for i, update in enumerate(updates):
            for name, delta in update.items():
                avg_update[name] += num_samples * self.adaptive_weighting[i] / sum(num_samples)
        
        # Update the baseline model weights
        global_grads = self.process_grad(avg_update)
