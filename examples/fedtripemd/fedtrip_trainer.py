        ...
        try:
            ...
            if callable(custom_train):
                ...
                for epoch in range(1, epochs + 1):
                    for batch_id, item in enumerate(train_dloader):
                        ...
                        if cnt == 0:
                            for name, params in self.model.named_parameters():
                                params.grad.data.add_(params.data - self.global_model[name], alpha=self.alpha / epochs)
                                params.grad.data.add_(self.last_model[name].to(self.device) - params.data, alpha=self.alpha / self.gap / epochs)
                            ...
                            for (name, params), value in zip(self.model.named_parameters(), self.update_direction.values()):
                                params.grad.data.add_(params.data - self.global_model[name].to(self.device), alpha=self.alpha / epochs)
                                params.grad.data.add_(self.last_model[name].to(self.device) - params.data, alpha=self.alpha / self.gap / epochs)
                                # if len(list(params.size())) > 1:
                                #     params.grad.data.add_(-params.grad.data.mean(dim=tuple(range(1, len(list(params.grad.data.size())))), keepdim=True))
                        cnt += 1
                    ...
                    self.last_model = deepcopy(self.model.state_dict())
